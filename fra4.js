...
answerOptions.addEventListener('click', function(event) {
  if (event.target.tagName === 'INPUT') {
    ...
    let isCorrectAnswer = selectedValue === 'value-4';
    let resultClass = isCorrectAnswer ? '.result.success' : '.result.error';
    let nextVideo;

    if (selectedValue === 'value-1' || selectedValue === 'value-3') {
      nextVideo = 'ani/fra4X+.mp4';
    } else if (selectedValue === 'value-2') {
      nextVideo = 'ani/fra4X-.mp4';
    } else {
      nextVideo = 'ani/fra4O.mp4';
    }
...
nextButton.addEventListener('click', function() {
  window.location.href = 'fra5.html';
});
